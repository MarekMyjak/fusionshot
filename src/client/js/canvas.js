...
class Canvas {
    constructor(params) {
        ...
        this.canvas.addEventListener('click', this.shootMissile, false);
    ...
    directionDown(event) {
        ...
        var self = this.parent; // have to do this so we are not using the canvas object
    ...
    keyInput(event) {
    	...
    	if (key === global.KEY_FIREFOOD && this.parent.reenviar) {
            ...
            this.parent.socket.emit('splitCell');
            this.parent.reenviar = false;
        }
